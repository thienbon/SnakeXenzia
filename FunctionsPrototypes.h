...
void highlightMenu();
void file(fstream& fs, string name);
// ====== NGUYEN MAU HAM ======
bool kt_game_over_mode_2()
{
	if (kt_gameover() == true)
...
void ket_qua_lua_chon()
{
	...
	cout << "Map: " << map_choice << endl;
	cout << "Snake head: " << snake_head << endl;
	cout << "Snake color: ";
	gotoXY(14, 4);
	if (snake_color == 1)
	{
...
void file( string name)
{
	...
	if (game_mode == 1)
		...
		mode = "Combat";
	fo.open("Score.txt", ios::out);
	fo << "Rank \t\t Username \t\t Game Mode \t\t Score \n";
	fo << i << "\t\t" << Pname << "\t\t" << mode << "\t\t" << diem << endl;
	if (game_over == true)
		i++;
}
